import Image from 'next/image'


export default function ContentSection4() {

    return (
        <section className="flex w-[1440px] h-[342px] bg-gray_03 items-center justify-center">
            <div className="w-[1220px] h-[122px] flex items-center justify-around">
                <h2 className='font-bold text-[24px]'>Наши партнёры по монетизации</h2>
                <section className="w-[633px] h-[71px] flex items-center justify-between">
                    <figure>
                        <Image
                        src="/images/ClickWave.png"
                        alt="ClickWave"
                        width={177}
                        height={54}
                        priority
                        />
                        <figcaption className="sr-only">ClickWave</figcaption>    
                    </figure>
                    <figure>
                        <Image
                        src="/images/Zyphronix.png"
                        alt="Zyphronix"
                        width={102}
                        height={71}
                        priority
                        />
                        <figcaption className="sr-only">Zyphronix</figcaption>    
                    </figure>
                    <figure>
                        <Image
                        src="/images/NovaSphere.png"
                        alt="NovaSphere"
                        width={192}
                        height={61}
                        priority
                        />
                        <figcaption className="sr-only">NovaSphere</figcaption>    
                    </figure>
                </section>
            </div>
        </section>
    )
}